...
/* ── Pure-JS SHA-256 fallback ──────────────────────────────────────────────
   ...
   On a plain-HTTP origin (e.g. a LAN IP during local dev) it is undefined, and
   without this fallback every password hash/verify would throw. This produces
   byte-for-byte the same digest as crypto.subtle('SHA-256'), so hashes stay
   compatible across secure and non-secure contexts. */
const K256 = [
...
export async function hashPwd(plain) {
  ...
  // Prefer the native crypto (fast, constant-time); fall back to pure JS when
  // crypto.subtle is unavailable (non-secure origin).
  ...
  return 'sha256:' + sha256HexJS(data);
}
