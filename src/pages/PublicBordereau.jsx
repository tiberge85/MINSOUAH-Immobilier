...
export default function PublicBordereau() {
  const { id } = useParams();
  const [state, setState] = useState('loading'); // loading | ok | notfound
  const [data, setData] = useState(null);

  useEffect(() => {
    getDoc(doc(db, 'workspaces', WS, 'bordereauVerify', id))
      .then((snap) => {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-surface rounded-2xl shadow-xl border border-outline-variant/20 p-6 text-center">
        ...
        <h1 className="font-black text-xl text-primary">Vérification de bordereau</h1>
        ...
        {state === 'notfound' && (
          <div className="mt-5">
            ...
            <p className="font-bold text-error">Bordereau introuvable ou non validé</p>
        ...
        {state === 'ok' && data && (
          <div className="mt-5 text-left">
            ...
            <div className="flex flex-col gap-2 text-sm bg-surface-container-low rounded-xl p-4">
              ...
              <Row label="Nombre de loyers" value={data.count} />
              <Row label="Montant" value={fmt(data.total)} />
