    ...
    <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-sm shadow-modal text-body-sm">
      ...
      {payload.map((p) => (
        <p key={p.dataKey} style={{ color: p.color }}>
          {p.name === 'revenus' ? 'Revenus' : 'Dépenses'}: {Number(p.value || 0).toLocaleString('fr-CI')} FCFA
        </p>
      ))}
    </div>
  );
                ...
                <ResponsiveContainer width="100%" height="100%">
                  {chartType === 'area' ? (
                    <AreaChart data={chartPeriod === '6 Mois' ? revenueData.slice(-6) : revenueData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                      ...
                      <YAxis tick={{ fill: '#817662', fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={(v) => Number(v || 0).toLocaleString('fr-CI')} width={78} />
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={annualData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fill: '#817662', fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={(v) => Number(v || 0).toLocaleString('fr-CI')} width={85} />
                  <Tooltip content={<CustomTooltip />} />
                  <Legend formatter={(v) => v === 'revenus' ? 'Revenus' : 'Dépenses'} />
