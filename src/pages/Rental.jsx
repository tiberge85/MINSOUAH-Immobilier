...
export default function Rental() {
  ...
  // Revenu propriétaire RECALCULÉ en direct depuis ses biens (loyers additionnés en
  // nombres) — répare aussi l'affichage des fiches dont le revenu stocké est corrompu
  // (montants collés en texte, ex. « 330000330000… »).
  const computeOwnerRevenue = (o) => {
    const ids = new Set(o.propertyIds || []);
    return (properties || [])
      .filter(p => ids.has(p.id) || normN(p.owner) === normN(o.name) || (p.ownerId != null && Number(p.ownerId) === Number(o.id)))
      .reduce((s, p) => {
        if (p.isBuilding) return s + (p.units || []).filter(u => u.status === 'Loué').reduce((a, u) => a + (Number(u.rent) || 0), 0);
        return s + (Number(p.rent) || 0);
      }, 0);
  };
  ...
  // Créer l'accès portail d'un locataire directement depuis sa fiche.
  const createTenantAccess = (t) => {
    if (!t.email || !t.email.trim()) {
      alert(`Ajoutez d'abord un e-mail à ${t.name} (bouton Modifier) pour pouvoir créer son accès au portail.`);
      return;
    }
    const exists = (state.users || []).some(u => (u.email || '').toLowerCase() === t.email.trim().toLowerCase());
    if (exists) {
      alert(`Un accès existe déjà pour ${t.email}.\nRetrouvez-le dans Paramètres → Utilisateurs.`);
      return;
    }
    offerCreateAccount(t.name, t.email, 'TENANT', t.id);
  };
            ...
            <div key={t.id} className="bg-surface rounded-2xl p-4 border border-outline-variant/20 shadow-sm group hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {canCreate && (
                    <button title="Créer l'accès au portail locataire" onClick={() => createTenantAccess(t)}
                      className="flex items-center justify-center w-9 h-9 rounded-lg text-on-surface-variant hover:bg-surface-container-high hover:text-green-700 transition-colors">
