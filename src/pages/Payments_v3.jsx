...
function buildArrearsRecoveredReportHTML(recoveredByTenant, recoveredTotal, orgSettings) {
  ...
  const fmtPaid = (p) => {
    if (p.paidDate && /^\d{4}-\d{2}-\d{2}/.test(p.paidDate)) {
      ...
      return isNaN(dt) ? p.paidDate : dt.toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' });
    }
    return p.paidDate || '—';
  };
  ...
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">
  <title>Rapport des Arriérés Recouvrés</title>
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{font-family:'Segoe UI',Arial,sans-serif;font-size:13px;color:#1a1a1a;background:#fff;padding:28px}
    @media print{body{padding:0}@page{margin:18mm 14mm}h2{page-break-before:auto}}
    ...
    .org-name{font-size:20px;font-weight:900;color:#16a34a}
    .kpis{display:flex;gap:14px;margin-bottom:20px;flex-wrap:wrap}
    .kpi{flex:1;min-width:130px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:12px 16px;text-align:center}
    .kpi-val{font-size:20px;font-weight:900;color:#16a34a}
    .kpi-lbl{font-size:10px;color:#14532d;margin-top:2px;text-transform:uppercase;letter-spacing:.05em}
    h2{font-size:14px;font-weight:800;color:#1a1a1a;margin:20px 0 10px;border-bottom:2px solid #bbf7d0;padding-bottom:6px}
    .footer{margin-top:24px;padding-top:12px;border-top:1px solid #e5e7eb;font-size:10px;color:#999;text-align:center}
  </style></head><body>
  <div class="header">
    <div style="display:flex;align-items:center;gap:12px">
    ...
    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px">
      <div style="background:#16a34a;color:#fff;padding:8px 18px;border-radius:20px;font-size:13px;font-weight:700;white-space:nowrap">${fCFA(recoveredTotal)} recouvrés</div>
      ${orgStamp ? `<img src="${orgStamp}" style="max-height:48px;max-width:48px;object-fit:contain;opacity:0.85"/>` : ''}
    </div>
  </div>
...
export default function Payments() {
  ...
  // Comparaison de libellé de mois tolérante aux accents/casse/espaces
  // (« Aout 2026 » ≡ « Août 2026 ») pour ne pas exclure à tort un paiement
  // dont le mois a été saisi avec une légère variante d'écriture.
  const normLabel = (s) => (s || '').toString().toLowerCase().trim().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/\s+/g, ' ');
  const monthPmts = payments.filter(p => normLabel(p.month) === normLabel(selectedMonth));
  ...
  // la clôture). Sans ce second volet, la carte "Impayés" affichait 0 F au mois en cours.
  // Aligné sur le dénominateur de recoveryRate pour rester cohérent avec le taux et le badge.
  const totalPending = (() => {
    ...
    // Correspondance robuste locataire ↔ contrat : par ID de locataire (résolu via
    // la fiche locataire quand le contrat n'en porte pas), ou par nom normalisé avec
    // INCLUSION (« BINEY JACQUELINE » ⊂ « Mme BINEY JACQUELINE épse HADJI »). Sans
    // cela, un paiement dont le nom porte un titre/mention d'épouse n'était pas
    // rattaché au contrat → le locataire restait compté en impayé malgré son paiement.
    const nameMatch = (a, b) => {
      a = norm(a); b = norm(b);
      if (!a || !b) return false;
      if (a === b) return true;
    ...
    const paidNames = monthPmts.map(p => norm(p.tenantName)).filter(Boolean);
    ...
    const rentFor = (name) => {
      const c = (contracts || []).find(c => nameMatch(c.tenant, name) && (c.status === 'Actif' || c.status === 'Expirant'));
      return c?.rent || 0;
    };
    ...
    (contracts || []).filter(c => c.status === 'Actif' || c.status === 'Expirant').forEach(c => {
      ...
      const cid = c.tenantId != null ? String(c.tenantId) : '';
      if (!name && !cid) return;
      if (contractRecorded(c)) return; // déjà un paiement ce mois (par ID ou nom)
      const tenant = (tenants || []).find(t => nameMatch(t.name, c.tenant) || (c.tenantId && String(t.id) === String(c.tenantId)));
      const psDate = tenant?.paymentStartDate ? new Date(tenant.paymentStartDate) : null;
      const psFirst = psDate && !isNaN(psDate.getTime()) ? new Date(psDate.getFullYear(), psDate.getMonth(), 1) : null;
  ...
  const recoveredArrears = useMemo(() => {
    ...
    return (payments || [])
      ...
      .map(p => {
        ...
        // Un « arriéré recouvré » = règlement d'un VRAI arriéré. On EXCLUT les
        // paiements normaux réglés un peu après la fin du mois (ex. Mme TOUTOUKPE,
        // à jour) : sinon un simple règlement tardif serait compté à tort.
        //  • arriéré explicite (onglet Arriérés) ou généré à la clôture (autoGenerated) ;
        //  • OU loyer d'un mois CLÔTURÉ réglé APRÈS la clôture (repère postCloture
        //    ou date de règlement postérieure) — inclut les paiements « normaux »
        //    encaissés après une clôture (ex. S2, MAG 2).
        void isPast;
        let recovered = false;
  ...
  }, [payments, now, closedMonthSet, monthClosures]);

  // Portée de l'affichage « Arriérés recouvrés » :
  //  • Mois SÉLECTIONNÉ CLÔTURÉ → UNIQUEMENT les NOUVEAUX règlements reçus APRÈS la
  //    clôture (repère postCloture ou date de règlement ≥ jour de clôture). C'est
  //    exactement « la liste des nouveaux paiements après clôture » demandée.
  //  • Mois non clôturé → arriérés recouvrés pendant le mois sélectionné.
  const recoveredScoped = useMemo(() => {
    const selClosure = (monthClosures || []).find(c => c.month === selectedMonth);
    const selClosureDate = selClosure?.closedAt ? new Date(selClosure.closedAt) : null;
    const selClosureDay = selClosureDate
      ? new Date(selClosureDate.getFullYear(), selClosureDate.getMonth(), selClosureDate.getDate())
      : null;
    return recoveredArrears.filter(p => {
      const d = parsePaidDate(p.paidDate);
      if (!d || isNaN(d.getTime())) return false;
      if (selClosureDay) {
        if (p.postCloture) return true;            // enregistré explicitement après la clôture
        return d >= selClosureDay;                 // réglé le jour de la clôture ou après
      }
      return d.getMonth() === selMonthDate.getMonth() && d.getFullYear() === selMonthDate.getFullYear();
    });
  }, [recoveredArrears, selMonthDate, monthClosures, selectedMonth]);
