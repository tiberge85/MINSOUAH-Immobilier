...
export default function OwnerPortal() {
  ...
  const exportPDF = () => {
    ...
    // Paiements listés : la période choisie si filtre actif, sinon UNIQUEMENT le
    // mois en cours (par mois de loyer) — on retire ainsi les loyers de juillet /
    // septembre qui n'ont pas à figurer dans les paiements « du mois » d'août.
    const pdfPayments = hasPeriod ? periodPayments : ownerPayments.filter(p => p.month === curMonthLbl);
...
<h2>Paiements ${hasPeriod ? `(${pdfPayments.length})` : `du mois — ${curMonthLbl} (${pdfPayments.length})`}</h2>
<table>
  <thead><tr><th>Propriété</th><th>Locataire</th><th>Mois</th><th>Montant</th><th>Statut</th></tr></thead>
  <tbody>${rows(pdfPayments)}</tbody>
</table>
